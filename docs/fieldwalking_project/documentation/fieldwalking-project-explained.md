...
### Symbology
The symbology of this layer is fairly simple and has been set to a transparent green colour to easily identify which areas have been covered. This layer is also set to automatically label based off the value in the Date field to give a visual indication of when each area was covered.
### Attributes
...
### Symbology
Much like the `areas_covered` layer, I have chosen a relatively simple transparent red symbology to as an easily recognisable indication of which areas were not available for survey. This layer is also set up with automatic labelling based off the `Date` field and the `Description` field so that the date and reason for the observation can be easily understood in the map canvas.
